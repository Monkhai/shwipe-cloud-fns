...
export async function db_getReceivedFriendRequests(userId: string): Promise<FriendRequest[]> {
  const pool = await getPool()
  const query = `SELECT * FROM ${FriendRequestsTable.NAME} WHERE ${FriendRequestsTable.RECEIVER_ID} = $1 AND ${FriendRequestsTable.STATUS} = '${FriendRequestsStatus.PENDING}'`
