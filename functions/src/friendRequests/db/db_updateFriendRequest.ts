...
export async function db_updateFriendRequest(userId: string, friendRequestId: string, status: FriendRequestsStatus) {
  try {
    ...
    const verifyQuery = `SELECT ${FriendRequestsTable.SENDER_ID} FROM ${FriendRequestsTable.NAME} WHERE ${FriendRequestsTable.ID} = $1`
    ...
    const updateQuery = `UPDATE ${FriendRequestsTable.NAME} SET ${FriendRequestsTable.STATUS} = $1 WHERE ${FriendRequestsTable.ID} = $2`
